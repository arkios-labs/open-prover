...
fn main() -> Result<()> {
    tracing_subscriber::fmt()
        ...
        .with_writer(io::stderr) // 👈 로그는 stderr로 출력
        .init();

    let agent_type = env::var("AGENT_TYPE")
        .map_err(|_| anyhow::anyhow!("Missing AGENT_TYPE env var"))?;

    let task_type = env::var("TASK_TYPE").context("Missing TASK_TYPE")?;

    info!("Running AGENT_TYPE={agent_type} with TASK_TYPE={task_type}");

    let command = Command::from_str(&task_type)?;

    let agent_provider = Box::new(EnvProvider { key: "AGENT_TYPE".to_string() });
    ...
    let result_bytes = command.apply(agent_ref, input_bytes)?;
