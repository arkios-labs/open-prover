...
use crate::tasks::{deserialize_obj, serialize_obj, Agent, ProveKeccakRequestLocal, SerializableSession};
...
fn test_e2e_start_proof_generation() -> Result<()> {
    ...
    while queue.len() > 1 {
        ...
        while i + 1 < queue.len() {
            ...
            let union = agent_ref
                .union(left.last().unwrap().clone(), right.last().unwrap().clone())
                .expect("Union failed");
    ...
    agent_ref.finalize(resolved, journal_bytes, image_id?)?;
