...
mod tests {
    ...
    async fn test_e2e_stark_proof_generation() -> anyhow::Result<()> {
        ...
        let serialized_succinct_receipts: Vec<Vec<u8>> = all_succinct_receipts
            .into_iter()
        ...
        assert!(
            !root_receipt_bytes.is_empty(),
            "Root receipt should not be empty"
        );
        ...
        for (i, keccak_req) in session.pending_keccaks.iter().enumerate() {
            ...
            assert!(
                keccak_receipt.claim.as_value().is_ok(),
        ...
        info!(
            "Step 3 done in {:?} with {} receipts",
            start_step_3.elapsed(),
            keccak_receipts.len()
        );
        ...
        info!(
            "Step 4 done in {:?}, final union size: {}",
            start_step_4.elapsed(),
            final_result.len()
        );
        ...
        assert!(
            !rollup_receipt.is_empty(),
            "Rollup receipt should not be empty"
        );
        ...
        assert!(
            ...
            "Final Groth16 receipt should have a claim"
        );
